 ...
 * @author Leo
 */
@ManagedBean(name = "messagesIn")
@SessionScoped
public class ReceivedMessageListBean implements Serializable {
    ...
    public ReceivedMessageListBean() {
    ...
    public String makeReceivedList_refresh(String recipientUsername) {
        showHidden = true;
        messageList = makeReceivedList(recipientUsername);
        return "/restricted/received_messages?faces-redirect=true?";
    }

    public DataModel<MessageBean> makeReceivedList(String recipientUsername) {
        UserDB recipientDB = new UserDB();
        DataModel<MessageBean> ret = null;

        List resultList = messageDB.getMessages(recipientUsername, 2);
